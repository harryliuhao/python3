# -*- coding: utf-8 -*-
"""
Created on Sat May 30 15:03:23 2020
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        ...
        if pick>self.difficulty:
            return True
        else:
            return False
    
    def getPossibleLetters(self,guessed):
        l=[c for c in LETTERS]
        possibleLetters=[]
        for c in l:
            if c not in guessed:
                if c in VOWELS:
    ...
    def getMove(self,category, obscuredPhrase, guessed):
        ...
        if self.smartCoinFlip():
            for c in reversed(self.SORTED_FREQUENCIES):
                if c in possibleLetters:
                    return c
        else:
...
guessed=[letter for letter in 'LDZIJQGKCU']
p1=WOFComputerPlayer('Jevis',4)     
print(p1)       
p0=WOFPlayer('orion')
p0.addMoney(32)
print(p0)
l=p1.getPossibleLetters(guessed)    

SORTED_FREQUENCIES='ZQXJKVBPYGFWMUCLDRHSNIOATE'
possibleLetters='MUCLDRHS'
for c in reversed(SORTED_FREQUENCIES):
    if c in possibleLetters:
        print(c)
        break
